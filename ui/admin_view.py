...
class AdminView(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            top,
            text="  •  Gestión de base de conocimiento médica",
            style="Muted.TLabel",
            background=PALETTE["surface"]
        ).pack(side="left", anchor="w")
        ...
        ttk.Button(
            ...
            command=self._guardar_y_recargar
        ).pack(side="right", padx=4)
    ...
    def _tab_enfermedades(self, nb):
        ...
        ttk.Label(
            left,
            text="Enfermedades registradas",
            style="H3.TLabel",
            background=PALETTE["surface"]
        ).pack(anchor="w", padx=10, pady=(10, 4))

        self.tree_enf = ttk.Treeview(
            left,
        ...
        ttk.Button(
            ...
            text="+ Nueva",
            style="Primary.TButton",
            command=self._nueva_enfermedad
        ).pack(side="left", padx=(0, 4))

        ttk.Button(
            btn_row,
            text="💾 Guardar enfermedad",
            style="Ghost.TButton",
            command=self._guardar_enfermedad
        ).pack(side="left", padx=(0, 4))
        ...
        ttk.Button(
            ...
            command=self._eliminar_enfermedad
        ).pack(side="left")
        ...
        ttk.Label(
            right,
            text="Síntomas asociados (peso 1-5):",
            style="H3.TLabel",
            background=PALETTE["surface"]
        ).grid(row=6, column=0, columnspan=2, sticky="w", padx=12, pady=(16, 4))

        self.tree_sint_enf = ttk.Treeview(
            right,
            columns=("sintoma", "peso"),
            show="headings",
            height=6
        )
        ...
        ttk.Button(
            ...
            style="Ghost.TButton",
            command=self._agregar_sintoma_enf
        ).pack(side="left", padx=(0, 4))
        ...
        ttk.Button(
            ...
            text="🗑 Quitar",
            style="Danger.TButton",
            command=self._quitar_sintoma_enf
        ).pack(side="left")
    ...
    def _nueva_enfermedad(self):
        self._enf_vars["enf_nombre"].set("")
        self._enf_vars["enf_desc"].set("")
        self._enf_vars["enf_sistema"].set(SISTEMAS[0])
        self._enf_vars["enf_tipo"].set(TIPOS[0])

        self.tree_sint_enf.delete(*self.tree_sint_enf.get_children())

        seleccion_actual = self.tree_enf.selection()
        if seleccion_actual:
            self.tree_enf.selection_remove(*seleccion_actual)

    def _guardar_enfermedad(self):
        ...
        if self.tree_enf.exists(nombre):
            self.tree_enf.selection_set(nombre)
            self.tree_enf.focus(nombre)
            self.tree_enf.see(nombre)
            self._on_select_enf()

        messagebox.showinfo(
            "Listo",
            f"Enfermedad '{nombre}' guardada correctamente."
        )
    ...
    def _eliminar_enfermedad(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar '{nombre}'?"):
            ...
            messagebox.showinfo("Listo", f"Enfermedad '{nombre}' eliminada correctamente.")
    ...
    def _tab_medicamentos(self, nb):
        ...
        ttk.Button(
            ...
            text="+ Nuevo",
            style="Primary.TButton",
            command=self._nuevo_medicamento
        ).pack(side="left", padx=(0, 4))
        ...
        ttk.Button(
            ...
            text="💾 Guardar",
            style="Ghost.TButton",
            command=self._guardar_medicamento
        ).pack(side="left", padx=(0, 4))
        ...
        ttk.Button(
            ...
            text="🗑 Eliminar",
            style="Danger.TButton",
            command=self._eliminar_medicamento
        ).pack(side="left")
        ...
        ttk.Label(
            right,
            text="Contraindicado con alergias:",
            style="H3.TLabel",
            background=PALETTE["surface"]
        ...
        ttk.Label(
            right,
            text="Contraindicado con condiciones:",
            style="H3.TLabel",
            background=PALETTE["surface"]
        ...
        ttk.Entry(
            right,
            textvariable=self._med_contra_c_var,
            width=40
    ...
    def _nuevo_medicamento(self):
        ...
        seleccion_actual = self.tree_med.selection()
        if seleccion_actual:
            self.tree_med.selection_remove(*seleccion_actual)
    ...
    def _tab_sintomas(self, nb):
        ...
        ttk.Label(
            card,
            text="Catálogo de síntomas del sistema",
            style="H3.TLabel",
            background=PALETTE["surface"]
        ...
        ttk.Entry(
            row,
            textvariable=self._sint_nuevo_var,
            width=22
        ...
        ttk.Button(
            ...
            text="+ Agregar",
            style="Primary.TButton",
            command=self._agregar_sint_cat
        ).pack(side="left", padx=(0, 4))
    ...
    def _tab_archivo(self, nb):
        ...
        ttk.Button(
            ...
            style="Ghost.TButton",
            command=self._refrescar_vista_pl
        ).pack(side="left", padx=(0, 6))

        ttk.Button(
            btn_row,
            text="📥 Cargar .pl externo",
            style="Ghost.TButton",
            command=self._cargar_pl_externo
        ).pack(side="left", padx=(0, 6))
        ...
        scroll_x = ttk.Scrollbar(card, orient="horizontal", command=self.txt_pl.xview)

        self.txt_pl.configure(
    ...
    def _guardar_y_recargar(self, silent=False):
        try:
            ...
            messagebox.showerror(
                "Error al guardar",
                f"No se pudo escribir el archivo .pl:\n{ex}"
            )
            return

        try:
            ruta = os.path.abspath(self.pl_ruta).replace("\\", "/")
            list(self.engine.prolog.query(f"load_files('{ruta}', [if(true)])."))
        except Exception:
            pass
